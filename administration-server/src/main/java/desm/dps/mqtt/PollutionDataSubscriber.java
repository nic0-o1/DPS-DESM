 ...
 * It deserializes incoming messages into {@link PollutionData} objects and persists them using the
 * injected {@link MeasurementRepository}. The component's lifecycle is managed by Spring.
 */
@Component
public class PollutionDataSubscriber implements MqttCallback {
    ...
    /** The full URL of the MQTT broker */
    @Value("${mqtt.broker.url:tcp://localhost:1883}")
    private String brokerUrl;

    /** The MQTT topic to subscribe to for pollution data. */
    @Value("${mqtt.topic.pollution:desm/pollution/data}")
    private String topic;

    /** A base name for the MQTT client ID, which will be made unique upon initialization. */
    @Value("${mqtt.client.id:adminServer}")
    private String baseClientId;

    /** The Quality of Service (QoS) level for the MQTT subscription. */
    @Value("${mqtt.qos:2}")
    private int qos;
    ...
    /**
     * Initializes the MQTT client after dependency injection is complete.
     * This method connects to the broker and subscribes to the pollution data topic.
     * It is automatically invoked by Spring due to the {@link PostConstruct} annotation.
     *
     * @throws RuntimeException if the MQTT connection or subscription fails, to prevent the
    ...
    public void start() {
        try {
            ...
            throw new RuntimeException("MQTT client initialization failed. See logs for details.", e);
        }
    }

    /**
     * Gracefully disconnects from the MQTT broker during application shutdown.
     * This method is automatically invoked by Spring due to the {@link PreDestroy} annotation.
     */
    @PreDestroy
    public void stop() {
        try {
    ...
    public void deliveryComplete(IMqttDeliveryToken token) {
        // This callback is not used because this component only subscribes to messages.
    }
}